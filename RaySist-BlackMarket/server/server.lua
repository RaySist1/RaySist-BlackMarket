...
local RepairCheck = {}
local busy = false
RegisterNetEvent('RaySist_blackMarket:server:requestConf', function()
   local src = source
   TriggerClientEvent("RaySist_blackMarket:client:Config", src, Config.Items)
end)
...
RegisterNetEvent('RaySist_blackMarket:server:try', function(item)
    local src = source
    if not busy then 
        busy = true
    ...
    TriggerClientEvent("RaySist_blackMarket:client:3", src)
    TriggerClientEvent("RaySist_blackMarket:client:Config", -1, Config.Items)
end)
